...
class AccesoDatos {
    ...
    public function getLogin($login){
        $resu=false;
        $stmt=$this->dbh->prepare("select * FROM user WHERE login = ? limit 1");
    ...
    public function getPass($login){
        $resu=false;
        $stmt=$this->dbh->prepare("select password FROM user WHERE login = ?");
        $stmt->bindParam(1,$login);
    ...
    public function getClientes ($primero,$cuantos,$ordenarpor):array {
        $tuser = [];
        // Crea la sentencia preparada
       // echo "<h1> $primero : $cuantos  </h1>";
        
        $stmt_usuarios  = $this->dbh->prepare("select * from Clientes $ordenarpor limit $primero,$cuantos");
